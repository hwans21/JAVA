...
public class ListQuiz02 {
	...
	public static void main(String[] args) {
		/*
		 1. User클래스를 은닉된 변수 name, age로 선언해서 구현
		 User클래스의 생성자는 모든 필드값을 받는 생성자 1개
		 
		 2. User 객체를 모음저장할 수 있는 ArrayList를 선언
		 
		 3. User 객체를 3개 생성해서 리스트에 추가하세요
		 
		 4. 리스트에 저장된 모든 이름, 나이를 for문으로 출력(향상for문)
		 
		 5. 리스트 내부 객체 중 이름이 "홍길동"이 있다면 그 객체를 삭제해 주세요
		 
		 */
		
		List<User> userList = new ArrayList<User>();
		userList.add(new User("홍길동",30));
		userList.add(new User("김환서",20));
		userList.add(new User("조성진",25));
