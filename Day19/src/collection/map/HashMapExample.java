...
public class HashMapExample {
	...
	public static void main(String[] args) {
		/*
		 ...
		 - Key값을 통해 Value를 참조하는 형태
		 - Key는 중복저장을 허용하지 않는다.
		 
		 */
		
		// Map은 Key, Value쌍을 이루기 때문에 멀티 제네릭을 작성
		...
		System.out.println(map.containsKey("삐약이"));
		System.out.println(map.containsKey("멍멍이"));
		
		//Map 내부의 값을 참조하는 법: get(key)
		String nick = "삐약이";
		if(map.containsKey(nick)) {
			System.out.printf("별명이 %s인 학생의 이름은 %s입니다.",nick,map.get(nick));
			
		}else
			System.out.println("그런 별명이 없습니다.'");
