...
public class Point {
	...
	public void inputPoints(List<Student> s) {
		/*
		 1. 학생 객체를 1개 생성합니다.
	...
	public void showAllPoints(List<Student> stu) { //학생객체가 들어있는 리스트가 와야 해요!
		/*
		 1. 리스트 안에 들어있는 학생 객체들의 정보를
		 반복문을 이용하여 하나씩 전체 출력해야 합니다.
		 한 명의 학생 정보를 출력하는 메서드는 이미 작성 했습니다.(outputStuInfo)
		...
		System.out.println("================================================================");
		System.out.printf("\t\t\t\t\t반 평균 : %.2f",totalAvg/stu.size());
...
//		}
	...
	public void modifyPoint(List<Student> students) {
		/*
		 ...
		 - 찾는 학번이 없을 시 검색하지 못했다는 메세지를 출력해주세요
		 */
		...
		for(Student stu :students) {
			if(stu.getStuId().equals(stuNum)) {
				System.out.printf("%s학생의 점수를 입력하세요\n", stu.getName());
				while(true) {
					try {
						System.out.print("국어: ");
						...
						sc.nextLine(); // 사용자가 입력한 잘못된 값을 걷기 위함1
	...
	public void deletePoint(List<Student> students) {
		/*
		  ...
		  학생 정보를 삭제할 때 "xxx님의 정보를 삭제합니까? [Y/N]"
...
//		for(int i=0; i<students.size()-1;i++) {
//			if(students.get(i).getStuId().equals(stuNum)) {
//				System.out.printf("%s님의 정보를 삭제하시겠습니까? [Y/N]",
//						students.get(i).getName());
//				String checkY = sc.next();
//				if(checkY.toLowerCase().equals("y")) {
//					students.remove(i);
...
//			
//		}// 향상for문도 됨?
//		if(!flag) System.out.println("해당 정보를 찾지 못했습니다.");
		...
		if(!flag) System.out.println("해당 학번과 일치하는 학생 정보가 없습니다.");
